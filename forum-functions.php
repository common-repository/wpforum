...
function forum_get_group_passwd($id){
	global $wpdb, $table_groups;
	return $wpdb->get_var("SELECT passwd FROM $table_groups WHERE id = $id"); 
}
...
function forum_get_threads($forum){
	...
	return $wpdb->get_results("SELECT * FROM $table_threads 
		WHERE forum_id = $forum AND status <> 'sticky' ORDER BY last_post DESC LIMIT $start, $tpp");
...
function forum_get_menu($what, $feed_link = '', $mark_as_solved = '', $mark_as_unsolved = '', $thread = ''){
	...
	if(forum_get_reg() == true){
		...
		if($user_ID){
			$luv = get_usermeta($user_ID, 'lastvisit');
			if($luv)
				$visit = "<span><br />You last visited: ".date(get_option('forum_date_format'), get_usermeta($user_ID, 'lastvisit'))."</span>";
			$wel .= "Welcome, $user_name. $visit";
			$menu .= "$m $logout | $prof | $search $newpost $solve";		
...
function forum_get_post_form($what, $location, $thread){
	global 	$user_ID, $login_link, $logout_link, $user_email,
			$profile_link, $reg_link, $search_link, $PLUGIN_PATH, $user_login, $user_url, $user_level, $PLUGIN_PATH;
	...
	$out .= "<table cellspacing='1' id='forum-reply' class='reply_table' >\n
		...
		<tr>\n";
		if(get_option('forum_captcha') == true and !user_can_moderate()){
			$out .= get_captcha();
		}
			$out .= "<td colspan='2'>\n
...
function get_captcha(){
	global $wpdb, $table_captcha, $PLUGIN_PATH;
    $possible = 'ABCEFGHJKMNPRSTVWXYZ123456789';
    $code = '';
    $i = 0;
      while ($i < 4) { 
	...
	$sql = "INSERT INTO $table_captcha (ip, code) VALUES('{$_SERVER['REMOTE_ADDR']}', '$code')";
	$wpdb->query($sql);
	$id = $wpdb->insert_id;

	$out .= "<tr><td><img src='$PLUGIN_PATH/captcha.php?captcha_id=$id'></td>";
	$out .= "<td>Please enter the code on the left.<br /><input id='security_code' name='security_code' type='text' /></td></tr>";
