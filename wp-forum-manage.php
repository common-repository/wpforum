...
if(isset($_GET['addgroupsubmit'])){
	...
	$max = $max + 1;
	$insert = "INSERT INTO $table_groups (name, sort, passwd) VALUES('$_GET[groupname]', '$max', '$_GET[passwd]')";
	$wpdb->query($insert);
...
if(isset($_GET['editgroupsubmit'])){
	...
	$table_groups = $table_prefix."forum_groups";
	$update = "UPDATE $table_groups SET name = '$_GET[groupname]', passwd =  '$_GET[passwd]' WHERE id = '$_GET[group]'";
	$wpdb->query($update);
	echo "<div class='updated fade'><p>Group changed successfully.</p></div>";	
}
...
if(isset($_GET['forumupdate_submit'])){
	...
	update_option('forum_captcha', $_GET['forum_captcha']);
...
function wpforumsettings(){
	echo "<h3>Options</h3><form action='' method='get'>
		...
		echo "/></td>
			...
			<tr>
				<th>Use <a href='http://en.wikipedia.org/wiki/Captcha'>Captcha</a> for unregistered users:</th>
			  	<td><input type='checkbox' name='forum_captcha' value='true'";
			 if(get_option('forum_captcha') == 'true') 
				echo "checked='checked'";
			echo "/> (Requires <a href='http://www.libgd.org/Main_Page'>GD</a> library)</td>
			</tr>";
...
function wpforumstructure(){
	...
	if(isset($_GET['action']) && ($_GET['action'] == "addgroup" || $_GET['action'] == "editgroup")){
		...
		if($_GET['action'] == "editgroup"){
			...
			$passwd = forum_get_group_passwd($_GET['group']);
		...
		<table border='0' cellpadding='3' cellspacing='3'>
			...
			<tr>
				<td align='right'>Password (optional):</td>
